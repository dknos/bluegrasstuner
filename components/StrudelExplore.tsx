...
import {
  ...
  fetchSong, fetchCoverCode, eefanoSourceUrl, Song,
...
const StrudelExplore: React.FC<Props> = ({ onClose, onLoad, loadSample }) => {
  ...
  const doLoadCover = async (title: string, id: string) => {
    setBusy(id);
    try { const code = await fetchCoverCode(id); onLoad(code, title); }
    catch (e) { setLoaded((s) => ({ ...s, [id]: 'err' })); }
    finally { setBusy(null); }
  };
        ...
        {tab === 'tracks' && (
          <div className="flex flex-col gap-4">
            ...
            <div>
              ...
              <p className="text-[9px] font-mono mb-2" style={{ color: GREEN_DIM }}>by their authors (credited) — load fetches the author's public share live; or open the original on strudel.cc</p>
              <div className="grid sm:grid-cols-2 gap-1.5">
                {COVERS.map((c) => (
                  <div key={c.title} className="flex items-center gap-2 px-2.5 py-2 rounded" style={{ background: 'rgba(255,255,255,0.04)', border: `1px solid ${BORDER}` }}>
                    ...
                    {loaded[c.id || ''] === 'err' && <span className="text-[9px] text-red-400 font-mono">failed</span>}
                    {extLink(c.sourceUrl, 'source ↗')}
                    {c.strudelUrl && extLink(c.strudelUrl, 'open ↗')}
                    {c.id && (
                      <button onClick={() => doLoadCover(c.title, c.id!)} disabled={busy === c.id}
                        className="px-2 py-1 rounded text-[10px] font-mono font-bold shrink-0"
                        style={{ background: busy === c.id ? '#2a352a' : GREEN, color: busy === c.id ? GREEN_DIM : '#08120a' }}>
                        {busy === c.id ? '…' : '▶ load'}
                      </button>
                    )}
                  </div>
